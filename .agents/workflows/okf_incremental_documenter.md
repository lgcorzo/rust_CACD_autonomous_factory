...
1. **Incremental Update via Git Diffs (No Full Regeneration):**
   ...
   - Execute a git inspection (`git diff HEAD~1 --name-only` or compare against the last indexing hash) to isolate files changed within the `src/` or `code/` directories.
...
2. **The OKF (Open Knowledge Format) Standard & Mandatory Relative Paths:**
   - **One Concept = One File:** Each script, class, module, or core runbook lives in its own independent Markdown file inside `.knowledge/` (e.g., `.knowledge/api-controller.md`).
...
1. **Detect Changes:** Run a lightweight git check to identify modified source files in `src/` or `code/`.
2. **Analyze AST/Structure:** Read only the modified files (and their immediate callers/callees if dependencies shifted).
3. **Draft / Mutate OKF Files:** - Update or generate the individual `.md` files under `.knowledge/`.
...
4. **Synchronize Index:** Refresh the central `.knowledge/index.md` navigation map to reflect any new or altered entity links.
