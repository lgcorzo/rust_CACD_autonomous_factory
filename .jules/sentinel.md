...
## 2026-02-18 - [Path Traversal in MCP Tools]
**Vulnerability:** Found a Critical Path Traversal vulnerability in `execute_code.py` and `run_tests.py` where user-controlled file paths (via LLM output) were joined with a sandbox directory without validation.
**Learning:** `os.path.join` does not prevent path traversal (e.g., `../`). Trusting LLM output for file operations is dangerous as prompt injection can lead to arbitrary file write/delete.
**Prevention:** Always resolve paths to absolute paths using `os.path.abspath` and verify that the resulting path starts with the intended sandbox directory path.
