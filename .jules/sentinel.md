...
## 2026-02-23 - [Path Traversal in MCP Tools]
**Vulnerability:** The `execute_code` MCP tool allowed writing files outside the sandbox via path traversal in `files_changed` JSON payload.
**Learning:** LLM-generated file paths must be treated as untrusted user input. Even with a sandbox directory, standard `os.path.join` does not prevent directory traversal if the input is malicious (e.g., `../../etc/passwd`).
**Prevention:** Always use a secure path joining utility (like `safe_join`) that resolves paths relative to a base directory and enforces confinement before performing file operations.
