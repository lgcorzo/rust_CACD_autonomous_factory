...
@pytest.mark.asyncio
async def test_execute_coding_task(mock_context):
    ...
    with patch("autogen_team.application.workflows.autonomous_mission.CoderAgent") as MockCoder:
        ...
        result = await execute_coding_task.fn(task_input, mock_context)
    
    assert result == {"status": "done"}
    mock_coder.execute_task.assert_called_once()

@pytest.mark.asyncio
async def test_plan(mock_context):
...
@pytest.mark.asyncio
async def test_fan_out_tasks(mock_context):
    ...
    with patch("autogen_team.application.workflows.autonomous_mission.develop_task_workflow") as mock_wf:
...
@pytest.mark.asyncio
async def test_aggregate_and_review(mock_context):
    mission_input = MissionInput(goal="goal", repository_path="/p")
    ...
    with (
        ...
        patch("autogen_team.application.workflows.autonomous_mission.ReviewerAgent") as MockReviewer,
    ):
        MockTester.return_value.run_tests = AsyncMock(return_value={"status": "passed", "report": "all ok"})
        MockReviewer.return_value.review_changes = AsyncMock(return_value=MagicMock(approved=True, comments=["looks good"]))
