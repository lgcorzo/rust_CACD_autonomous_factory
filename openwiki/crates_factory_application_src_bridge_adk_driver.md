...
* `apply_patch(self (Self), _mission_id (&str), patch (&SurgicalPatch)) -> Result<ExecutionResult, FactoryError>`: Internal helper logic.
* `verify_syntax(self (Self), _file_path (&std::path::Path)) -> Result<bool, FactoryError>`: Internal helper logic.
...
class NativeADKDriver {
    -apply_patch(self: Self, _mission_id: &str, patch: &SurgicalPatch) Result<ExecutionResult, FactoryError>
    -verify_syntax(self: Self, _file_path: &std::path::Path) Result<bool, FactoryError>
}
