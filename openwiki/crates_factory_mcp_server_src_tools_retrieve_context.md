...
* `map_stacktrace_to_ast(self (Self), query (&str)) -> anyhow::Result<String>`: Internal helper logic.
* `push_osr_metric(self (Self), _metric (&factory_core::OsrMetric)) -> anyhow::Result<()>`: Internal helper logic.
* `search(self (Self), _query (&str)) -> anyhow::Result<String>`: Internal helper logic.
...
class ManualMockR2rClient {
    -map_stacktrace_to_ast(self: Self, query: &str) anyhow::Result<String>
    -push_osr_metric(self: Self, _metric: &factory_core::OsrMetric) anyhow::Result<()>
    -search(self: Self, _query: &str) anyhow::Result<String>
}
