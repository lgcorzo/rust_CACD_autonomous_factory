# LLMOps Python Package

Welcome to the LLMOps Python Package wiki. This package provides tools and utilities for LLM Operations, structured using **Domain-Driven Design (DDD)** principles.

## LLMOps Lifecycle Documentation

> [!IMPORTANT]
> These documents provide a comprehensive view of the project's business, data, architecture, and requirements context.

1. [**Business Understanding**](Business_Understanding.md): Objectives, success metrics, and project plan.
2. [**Data Understanding**](Data_Understanding.md): Data sources, quality analysis, and exploration.
3. [**Solution Architecture Report**](Solution_Architecture_Report.md): Detailed architectural components and repository mapping.
4. [**SRS Index (Package Requirements)**](SRS_Index.md): Central hub for all package-level requirements.
5. [**Test Plan & Report**](Test_Plan_Report.md): Verification strategy and evaluation results.

## Technical Architecture

- [**Architecture Overview (DDD)**](Architecture.md): Overview of the layers, domains, and project structure.

## Documentation Stories

### Application Layer

- [Base Stories (Job Context)](Base_stories.md)
- [Evaluations Stories](Evaluations_stories.md)
- [Explanations Stories](Explanations_stories.md)
- [Inference Stories](Inference_stories.md)
- [Inference Stories (Hatchet Async)](Inference_Async_Hatchet_stories.md)
- [Promotions Stories](Promotions_stories.md)
- [Training Stories](Training_stories.md)
- [Tuning Stories](Tuning_stories.md)

### Domain Layers

- [Models Stories](Models_stories.md)
- [Metrics Stories](Metrics_stories.md)
- [Schemas Stories](Schemas_stories.md)

### Infrastructure Layer

- [Configs Stories](Configs_stories.md)
- [OS Variables Stories](OSvariables_stories.md)
- [Services Stories](Services_stories.md)
- [Settings Stories](Settings_stories.md)
- [Signers Stories](Signers_stories.md)
- [Searchers Stories](Searchers_stories.md)
- [Splitters Stories](Splitters_stories.md)
- [Scripts Stories](Scripts_stories.md)

### Data Access Layer

- [Datasets Stories](Datasets_stories.md)
- [Registries Stories](Registries_stories.md)

---

- [Backlog LLMOps Autogen](backlog_llmops_autogen.md)
