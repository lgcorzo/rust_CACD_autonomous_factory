use crate::protocol::CallToolResult;
use async_trait::async_trait;
use serde_json::Value;

#[cfg_attr(test, mockall::automock)]
#[async_trait]
pub trait Tool: Send + Sync {
    fn name(&self) -> String;
    fn description(&self) -> String;
    fn input_schema(&self) -> Value;

    async fn call(&self, params: Value) -> anyhow::Result<CallToolResult>;
}

pub mod bridge;
pub mod deep_research_tool;
pub mod execute_code;
pub mod get_factory_status;
pub mod index_code;
pub mod inspect_kafka_topic;
pub mod launch_sandbox_pod;
pub mod list_minio_buckets;
pub mod list_minio_objects;
pub mod plan_mission;
pub mod retrieve_context;
pub mod run_tests;
pub mod search_jira;
pub mod security_review;
pub mod spec_kit_tasks_to_issues;
pub mod spec_kit_tool;
pub mod update_mission_status;

pub use bridge::BridgeTool;
pub use deep_research_tool::DeepResearchTool;
pub use execute_code::ExecuteCodeTool;
pub use get_factory_status::GetFactoryStatusTool;
pub use index_code::IndexCodeTool;
pub use inspect_kafka_topic::InspectKafkaTopicTool;
pub use launch_sandbox_pod::LaunchSandboxPodTool;
pub use list_minio_buckets::ListMinioBucketsTool;
pub use list_minio_objects::ListMinioObjectsTool;
pub use plan_mission::PlanMissionTool;
pub use retrieve_context::RetrieveContextTool;
pub use run_tests::RunTestsTool;
pub use search_jira::SearchJiraTool;
pub use security_review::SecurityReviewTool;
pub use spec_kit_tasks_to_issues::SpecKitTasksToIssuesTool;
pub use spec_kit_tool::SpecKitTool;
pub use update_mission_status::UpdateMissionStatusTool;
