...
use factory_core::executor::{CodeSurgeryExecutor, ExecutionResult, SurgicalPatch};
use std::fs;
...
impl CodeSurgeryExecutor for NativeADKDriver {
    async fn apply_patch(
        ...
        let content =
            fs::read_to_string(&full_path).map_err(|e| FactoryError::IoError(e.to_string()))?;
        ...
        fs::write(&full_path, updated_content).map_err(|e| FactoryError::IoError(e.to_string()))?;
